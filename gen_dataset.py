...
for c in range(1, 8):
    ...
    for i in range(1, 3):
        for j in range(1, 201):
            ...
            X, sample_rate = librosa.load('../Dataset_tess/C{2}/C{2}_{0}_{1}.wav'.format(i, j, c),
                                          res_type='kaiser_fast', sr=22050*2, offset=0.5, duration=2.5)
            data.append(X)
    ...
    np.savetxt('Dataset/ORIGINAL/wav_data_c{}.data'.format(c), data_matrix, delimiter=',', fmt="%s")
